...
class ICFAnalyzer:
    ...
    def _find_alpha_onset_index(self, threshold: float = 0.15) -> int:
        ...
        logger.info(
            f"Alpha onset: t={self.data.time[onset]:.3f} ns "
    ...
    def analyze_implosion_phase(self):
        ...
        if self.data.velocity is not None:
            ...
            self.data.alpha_onset_time_ns = float(self.data.time[alpha_onset_idx])
    ...
    def _compute_implosion_velocity_rhino(self):
        ...
        # Find turning point: FIRST significant local maximum of v_shell(t).
        #
        # Naive argmax over pre-stagnation catches the wrong peak. The
        # rho>peak/e shell mask becomes degenerate at late times --
        # focuses on a few super-compressed zones whose sqrt(2*KE/m)
        # blows up just before bang. argmax then picks that convergence
        # spike (~1600 km/s on WT_cthomas) instead of the in-flight peak
        # (~400 km/s). Will's "turning point" is the FIRST d/dt = 0
        # crossing -- the in-flight maximum, before the convergence
        # phase starts collapsing the shell definition.
        #
        # Algorithm:
        #   1. Restrict search to t < t_stag - buffer (excludes the
        #      shell-collapse window where v_shell is misleading).
        #   2. Use scipy.signal.find_peaks with a prominence threshold
        #      to ignore numerical jitter.
        #   3. Take the FIRST peak in that filtered window.
        #   4. Fallbacks: argmax over the in-flight window if no peak
        #      is found.
        ...
        # Buffer: exclude the last 1.5 ns before stagnation, where the
        # shell mask has collapsed onto super-compressed core zones.
        STAG_BUFFER_NS = 1.5
        end_t = float(stag_t) - STAG_BUFFER_NS if stag_t > 0 \
            else float(self.data.time[max(stag_idx - 1, 0)])
        end_idx = int(np.argmin(np.abs(self.data.time - end_t)))
        if end_idx <= 5:
            end_idx = max(int(0.75 * stag_idx), 5)
        end_idx = min(end_idx, n_times - 1)
        if end_idx <= 1:
            return

        v_window = v_shell[:end_idx + 1]
        if v_window.max() <= 0:
            return

        # First significant local max
        try:
            from scipy.signal import find_peaks
            prominence_threshold = 0.10 * float(v_window.max())
            peaks, _ = find_peaks(v_window, prominence=prominence_threshold)
        except Exception:
        ...
        if len(peaks) > 0:
            ...
            # Fallback: argmax over the buffered window. Without the
            # late-time collapse, this is the in-flight maximum.
            peak_idx = int(np.argmax(v_window))
...
# ------------------------------------------------------------------
    ...
    def _compute_inner_cr_history(self):
        """Build the inner-surface convergence-ratio history CR_in(t) = R_in(0) / R_in(t).

        The inner surface is the hot-spot / gas boundary at Lagrangian zone
        index ``ri[t, 0]`` -- the same surface used by the stagnation-CR
        convention (CLAUDE.md convention 12).  Populates
        ``self.data.cr_inner_history`` (np.ndarray over time, NaN where the
        radius is undefined) and ``self.data.cr_inner_R0`` (the t=0
        inner-surface radius, cm).

        Single-region targets (CH-only slab/sphere flux-limiter tests) have no
        gas cavity to converge to -- both attributes are set to None and the
        method returns None.

        Convention matches RHINO ``SphericalSimulation.cr_inner`` so the two
        codes clock the implosion identically.
        """
        ri = self.data.region_interfaces_indices
        zbnd = self.data.zone_boundaries
        time = self.data.time

        if ri is None or zbnd is None or time is None or ri.shape[1] < 2:
            self.data.cr_inner_history = None
            self.data.cr_inner_R0 = None
            return None

        try:
            R0 = float(zbnd[0, int(ri[0, 0])])
            nt = len(time)
            R_in = np.empty(nt)
            for t in range(nt):
                R_in[t] = zbnd[t, int(ri[t, 0])]
            with np.errstate(divide='ignore', invalid='ignore'):
                cr = np.where(R_in > 0, R0 / R_in, np.nan)
            self.data.cr_inner_history = cr
            self.data.cr_inner_R0 = R0
            logger.info(f"Inner-CR clock: R_in(0)={R0:.4f} cm, "
                        f"max CR_in={np.nanmax(cr):.2f}")
            return cr
        except Exception as e:
            logger.warning(f"Could not compute inner-CR history: {e}")
            self.data.cr_inner_history = None
            self.data.cr_inner_R0 = None
            return None

    def time_index_at_inner_cr(self, target_cr, interpolate=True):
        """First time index at which the inner convergence ratio reaches ``target_cr``.

        Lazily computes the inner-CR history on first use.

        Args:
            target_cr: Target inner convergence ratio (e.g. 1.5, 3.5).
            interpolate: If True, also return the fractional position between
                the bracketing indices for sub-timestep interpolation.

        Returns:
            ``(i, frac)`` where ``i`` is the first index with
            ``CR_in[i] >= target_cr`` and ``frac`` in [0, 1) locates the
            crossing between ``i-1`` and ``i``.  Returns None if the inner CR
            never reaches ``target_cr`` (or the target is undefined for this run).
        """
        cr = getattr(self.data, 'cr_inner_history', None)
        if cr is None:
            cr = self._compute_inner_cr_history()
        if cr is None:
            return None

        valid = np.isfinite(cr)
        reached = valid & (cr >= target_cr)
        if not np.any(reached):
            logger.warning(f"Inner CR never reaches {target_cr:.3g} "
                           f"(max CR_in = {np.nanmax(cr):.2f})")
            return None

        i = int(np.argmax(reached))   # first True
        if not interpolate or i == 0:
            return (i, 0.0)

        cr_lo, cr_hi = cr[i - 1], cr[i]
        if not np.isfinite(cr_lo) or cr_hi == cr_lo:
            return (i, 0.0)
        frac = (target_cr - cr_lo) / (cr_hi - cr_lo)
        frac = float(np.clip(frac, 0.0, 1.0))
        return (i, frac)

    def time_at_inner_cr(self, target_cr):
        """(Interpolated) simulation time at which the inner convergence ratio
        first reaches ``target_cr``.

        Args:
            target_cr: Target inner convergence ratio.

        Returns:
            The crossing time in ns, or None if never reached.
        """
        hit = self.time_index_at_inner_cr(target_cr)
        if hit is None:
            return None
        i, frac = hit
        time = self.data.time
    ...
    def value_at_inner_cr(self, series, target_cr):
        """Sample a per-timestep series at the moment the inner convergence
        ratio first reaches ``target_cr`` (linear interpolation between the
        bracketing timesteps).

        Generic enabler for the "at CR" family of metrics (ablation pressure at
        CR 3.5, adiabat at CR 1.5, IFAR at CR 1.5, sound speed at CR 1.5),
        matching Will Trickey's convention.

        Args:
            series: 1-D array aligned with ``self.data.time``.
            target_cr: Target inner convergence ratio.

        Returns:
            The interpolated value, or None if the CR is never reached or the
            series is unavailable.
        """
        hit = self.time_index_at_inner_cr(target_cr)
        if hit is None or series is None:
            return None
        i, frac = hit
        series = np.asarray(series)
        if i == 0 or frac == 0.0:
            return float(series[i])
        return float(series[i - 1] + frac * (series[i] - series[i - 1]))
    ...
    def compute_performance_metrics(self):
        ...
        # no gas cavity to converge to — skip CR computations entirely.
        ri = self.data.region_interfaces_indices
        has_fuel_interface = ri is not None and ri.shape[1] >= 2

        # Stagnation CR = R0 / R_hs_at_stagnation (CLAUDE.md convention 12)
        if (has_fuel_interface
                and self.data.zone_boundaries is not None
                and self.data.stag_time > 0):
            stag_idx = np.argmin(np.abs(self.data.time - self.data.stag_time))
            R0 = self.data.zone_boundaries[0, int(ri[0, 0])]
            R_hs = self.data.zone_boundaries[stag_idx, int(ri[stag_idx, 0])]
            if R_hs > 0:
        ...
        if (pv_idx is not None
            ...
            if abl_radius is not None and float(abl_radius[pv_idx]) > 0:
                ...
                Rf = self.data.zone_boundaries[pv_idx, int(ri[pv_idx, 0])]
            if Rf > 0:
